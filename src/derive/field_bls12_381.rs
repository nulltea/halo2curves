        ...
        impl From<$field> for i128 {
            fn from(value: $field) -> i128 {
                ...
                if tmp[2] == 0 && tmp[3] == 0 {
                    ...
                    let (a1, _) = sbb($modulus.0[1], tmp[1], borrow as u64);
    ...
    ($field:ident, $modulus:ident, $inv:ident, $field_type:ident) => {
        ...
        impl $field {
            ...
            pub const fn sub(&self, rhs: &Self) -> Self {
                ...
                let (d1, borrow) = sbb(self.0[1], rhs.0[1], borrow as u64);
                ...
                let (d1, carry) = adc(d1, $modulus.0[1] & borrow, carry as u64);
            ...
            pub const fn neg(&self) -> Self {
                ...
                let (d1, borrow) = sbb($modulus.0[1], self.0[1], borrow as u64);
    ...
    ($field:ident, $modulus:ident, $inv:ident, sparse) => {
        impl $field {
            ...
            pub(crate) const fn montgomery_reduce(
                ...
                let (mut r7, mut carry2) = adc(r7, carry, carry2 as u64);
    ...
    ($field:ident, $modulus:ident, $inv:ident, dense) => {
        impl $field {
            ...
            pub(crate) const fn montgomery_reduce(
                ...
                (r7, borrow) = sbb(r7, $modulus.0[1], borrow as u64);
